...
export default (book) => 'export default ' + JSON.stringify({
  info: {...book.info},
  entities: objectMap(book.entities, (rawEntity, key)=>{
    ...
    // Parse links
    ;(data.match(/\{link \w+:(\w|@)+\}/g) || []).forEach( (match) => {
      ...
      data = data.replace(match, `<lgc-link to="${number}">${title}</lgc-link>`);
    })

    // Replace every code inside {{ }} with text
    ;(data.match(/\{\{(?:(?!\}\})[^])*(\}\})/g) || []).forEach( (match) => {
      data = data.replace(match, stripHtml(match));
    })

    // Parse if
    ;(data.match(/\{\{if(\s)*\"(?:(?!\}\})[^])+\"(\s)*(\}\})/g) || []).forEach( (match) => {
      const condition = match.substring(match.indexOf('"') + 1, match.lastIndexOf('"'))
      data = data.replace(match, `<lgc-if v-if="${condition}">`);
    })

    // Parse html
    ;(data.match(/\{\{(\s)*\<(?:(?!\>( )*\}\})[^])+\>(\s)*(\}\})/g) || []).forEach( (match) => {
      const html = match.substring(match.indexOf('<'), match.lastIndexOf('>') + 1)
      data = data.replace(match, html);
    })


    const entity = {data}
    if(rawEntity.title && rawEntity.title !== '') entity.title = rawEntity.title
    return entity
  }),
